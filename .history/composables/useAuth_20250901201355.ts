...
export const useAuth = () => {
  ...
  const handleOAuthCallback = async (code: string, state: string, provider: 'google' | 'microsoft'): Promise<void> => {
    try {
      ...
      if (response.success && response.data) {
        setTokens(response.data.accessToken, response.data.refreshToken, true) // Remember OAuth logins
        user.value = response.data.user

        // Get redirect URL from state or use default
        const redirectTo = process.client ? sessionStorage.getItem('auth_redirect') || '/dashboard' : '/dashboard'
        if (process.client) {
  ...
  const generateOAuthUrl = (provider: 'google' | 'microsoft'): string => {
    const state = Math.random().toString(36).substring(2, 15)
    const currentRoute = process.client ? window.location.pathname : '/'
    
    if (process.client) {
      sessionStorage.setItem('oauth_state', state)
      sessionStorage.setItem('auth_redirect', currentRoute)
    }
  ...
  const socialLogin = async (provider: 'google' | 'microsoft'): Promise<void> => {
    ...
    // Note: isLoading.value = false is handled after redirect in the callback
  }
  ...
  return {
    ...
    fetchUser,
    initializeAuth,
    handleOAuthCallback // Export for use in callback pages
  }
}