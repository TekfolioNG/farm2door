...
export const useAuth = () => {
  // --- State management ---
  const user = useState<User | null>('auth.user', () => null) || ref(null)
  const isLoggedIn = computed(() => !!(user && user.value))
  ...
  const signup = async (credentials: SignupCredentials): Promise<void> => {
    try {
      ...
      if (response.success) {
        await login({ email: credentials.email, password: credentials.password, rememberMe: false })
  ...
  // --- Return API ---
  return {
    user: user, // keep it writable ref, avoids issues during SSR
    isLoggedIn,
