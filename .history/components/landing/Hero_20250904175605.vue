...
// Or if you want to check auth status and handle accordingly:
const enterFarmMarket = () => {
  // Check if user is logged in
  const { isLoggedIn } = useAuth()

  if (isLoggedIn.value) {
    // User is logged in, go to market
    navigateTo('/market')
  } else {
    // User not logged in, they can still browse market
    // but will see prompts to sign in for purchases
    navigateTo('/market')
  }
}
