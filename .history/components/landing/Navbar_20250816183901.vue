...
const menuitems = [
  { title: "Home", path: "/" },
  { title: "About Wilson", path: "/#" },
  { title: "Clarity Sessions", path: "/#" },
  { title: "Training", path: "/#" },
  { title: "My Books", path: "/#" },
];
...
const textColorClass = 'text-white hover:text-black hover:bg-white';
const progressBarColor = 'bg-white';
</script>
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center justify-between py-6">
        ...
        <NuxtLink to="/" class="inline-block">
          <img src="/wilsonsamson-logo.png" alt="wilsonsamson Logo"
            class="h-16 md:h-20 drop-shadow-lg brightness-0 invert" />
        </NuxtLink>
        ...
        <div class="hidden md:flex items-center w-full">
            ...
            <ul class="flex space-x-6 lg:space-x-8">
              <li v-for="item in menuitems" :key="item.title" class="relative">
                <NuxtLink :to="item.path" :class="[
                  'text-sm lg:text-base transition-all duration-300 flex items-center whitespace-nowrap font-medium text-white/50 hover:text-white hover:scale-105 hover:font-semibold relative group',
          ...
          <div class="hidden md:flex items-center ml-auto">
            <NuxtLink to="/contact"
              ...
              <div class="absolute bottom-0 right-4 h-0.5 bg-black rounded-full transition-all duration-300 ease-out"
                :class="[
                  isContactHovered ? 'w-8 opacity-100' : 'w-0 opacity-0'
                ]"></div>

              <!-- Ripple effect overlay -->
              <span v-if="isContactClicked" class="absolute inset-0 bg-white rounded-md animate-ping"></span>
            </NuxtLink>
          </div>
        </div>
        ...
        <div class="md:hidden flex items-center">
          <button @click="open = !open"
            class="focus:outline-none text-white/50 hover:text-white transition-colors duration-200">
