<template>
  <section class="relative w-screen h-screen overflow-hidden bg-zinc-900">
    ...
    <div
      class="absolute top-0 left-1/2 transform -translate-x-1/2 w-96 h-96 bg-gradient-radial from-amber-400/10 via-amber-600/5 to-transparent rounded-full blur-3xl" />

    <!-- Floating particles -->
    <div class="absolute inset-0">
      <div v-for="i in particleCount" :key="i" class="absolute w-1 h-1 bg-amber-400/30 rounded-full animate-pulse"
        :style="getParticleStyle(i)" />
    </div>

    <!-- Main Content -->
    <div
      class="relative z-10 w-full h-full flex flex-col justify-center items-center px-4 sm:px-6 md:px-16 lg:px-24 xl:px-32 mobile-content-spacing">

      <!-- Logo & Tagline -->
      <header class="text-center mb-16 transition-all duration-2000 mt-16 md:mt-0"
      ...
      <div class="relative w-full max-w-7xl h-96 hidden md:block">
        <Transition v-for="(category, index) in categories" :key="category.id" name="category-float" appear
          :style="{ transitionDelay: `${index * 200}ms` }">
          <div v-if="isLoaded" class="absolute transform transition-all duration-300 cursor-pointer group" :style="{
            ...category.position,
            ...
            <div class="relative">
              <div
                ...
                <div
                  class="text-4xl mb-3 transform transition-transform duration-300 group-hover:scale-110 floating-animation">
                  {{ category.icon }}
                </div>
      ...
      <div class="grid grid-cols-2 gap-4 w-full max-w-md mb-12 md:hidden">
        <div v-for="(category, index) in mobileCategories" :key="category.id"
          ...
          :class="isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'"
          :style="{ transitionDelay: `${index * 100}ms` }" @click="selectCategory(category)">
      ...
      <div class="flex flex-col items-center space-y-4 transition-all duration-2000"
        :class="isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'" style="transition-delay: 800ms">
        ...
        <button
          ...
          @click="enterFarm" aria-label="Enter Farm - Sign up or login">
          <span class="relative z-10">Enter Farm</span>
          <div
            class="absolute inset-0 bg-gradient-to-r from-emerald-400 to-emerald-500 rounded-full opacity-0 group-hover:opacity-20 transition-opacity duration-300" />
        </button>
        ...
        <button
          ...
          @click="browseAsGuest" aria-label="Browse products without signing up">
          Browse as guest
        </button>
