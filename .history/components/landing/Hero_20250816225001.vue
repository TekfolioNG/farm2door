    ...
    <div
      ...
      <div class="w-full max-w-4xl mx-auto text-center space-y-8">
        ...
        <div
          ...
          :class="isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'">
          <!-- Main Featured Image -->
          <div class="absolute inset-2 rounded-full overflow-hidden shadow-xl z-20">
            <transition name="fade" mode="out-in">
              <img :key="featuredProduct.id" :src="featuredProduct.image" :alt="featuredProduct.alt"
                class="w-full h-full object-contain transition-all duration-1000 hover:scale-105" />
            </transition>
            <div class="absolute inset-0 bg-gradient-to-b from-black/5 to-transparent" />
          </div>

          <!-- Decorative Elements -->
          <div class="absolute top-4 right-4 w-3 h-3 bg-[#9dcd5a] rounded-full opacity-80"></div>
          <div class="absolute bottom-4 left-4 w-2 h-2 bg-[#7ba54a] rounded-full opacity-60"></div>
        </div>
        ...
        <div class="transition-all duration-1000 transform delay-500"
          ...
          <button @click="enterFarmMarket"
            class="relative px-6 py-3 sm:px-8 sm:py-3 md:px-10 md:py-4 bg-[#7ba54a] hover:bg-[#6a9342] text-white font-['Barlow_Condensed'] font-semibold text-base sm:text-lg rounded-lg transition-all duration-300 hover:scale-105 shadow-[#7ba54a]/20 focus:outline-none focus:ring-2 focus:ring-[#7ba54a] focus:ring-offset-2 dark:focus:ring-offset-gray-900 animate-gentle-bob hover:animate-none">
            Enter Farm Market
          </button>
        </div>
...
import { Home, Leaf, Truck } from 'lucide-vue-next';
...
/* Gentle bobbing animation for button */
