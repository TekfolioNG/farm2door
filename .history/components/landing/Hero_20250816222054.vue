  ...
  <section
    ...
    <div
      ...
      <div class="w-full max-w-4xl mx-auto text-center space-y-8">
        ...
        <!-- Product Gallery -->
        <div class="relative w-full h-48 sm:h-56 md:h-64 mt-8 mb-12 transition-all duration-1000 transform delay-300"
          :class="isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'">
          <!-- Main Featured Image -->
          <div
            class="absolute inset-0 rounded-full overflow-hidden shadow-xl z-20 animate-gentle-float border-4 border-white/20">
            <img :src="featuredProduct.image" :alt="featuredProduct.alt"
              class="w-full h-full object-contain transition-all duration-1000 hover:scale-105 p-4" />
            <div class="absolute inset-0 bg-gradient-to-b from-black/10 to-transparent" />
          </div>
        ...
        <div class="transition-all duration-1000 transform delay-500"
          ...
          <button @click="enterFarmMarket"
            class="relative px-8 py-3 sm:px-10 sm:py-4 bg-[#7ba54a] hover:bg-[#5d8238] text-white font-['Barlow_Condensed'] font-semibold text-lg rounded-lg transition-all duration-300 hover:scale-105 hover:shadow-lg shadow-[#7ba54a]/30 focus:outline-none focus:ring-2 focus:ring-[#7ba54a] focus:ring-offset-2 dark:focus:ring-offset-gray-900 animate-occasional-bob">
            Enter Farm Market
          </button>
        </div>
        ...
        <div
          ...
          <div class="flex items-center">
            <div class="w-2 h-2 bg-[#9dcd5a] rounded-full mr-2"></div>
            <span class="font-['Barlow_Condensed'] font-medium">Chemicals-free</span>
          </div>
          <div class="flex items-center">
            <div class="w-2 h-2 bg-[#9dcd5a] rounded-full mr-2"></div>
...
const startProductRotation = () => {
  setInterval(() => {
    ...
    const imageContainer = document.querySelector('.animate-gentle-float');
    if (imageContainer) {
      imageContainer.classList.add('opacity-0', 'scale-95');
...
/* Occasional bob animation for button */
@keyframes occasional-bob {
...
.animate-occasional-bob {
  animation: occasional-bob 15s ease-in-out infinite;
}
...
/* Ensure images fit nicely in their containers */
img {
  width: 100%;
  height: 100%;
  object-fit: contain;
...
/* Smooth transition for product changes */
.opacity-0 {
  opacity: 0;
