<template>
  <section
    ...
    <div
      class="relative z-10 w-screen min-h-screen flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 pt-32 pb-16">
      <!-- Hero Content -->
      <div class="w-full max-w-4xl mx-auto text-center space-y-8">
        ...
        <div class="space-y-4 transition-all duration-1000 transform"
          ...
          <h2
            class="text-2xl sm:text-3xl md:text-4xl font-bold leading-tight tracking-tight text-gray-900 dark:text-white">
        ...
        <div class="w-screen -mx-4 sm:-mx-6 lg:-mx-8 mt-6 mb-8 transition-all duration-1000 transform delay-600"
          :class="isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'">
          <div class="overflow-hidden">
            <div class="animate-scroll-categories flex space-x-8 whitespace-nowrap py-2">
              <span v-for="category in [...foodCategories, ...foodCategories]" :key="`${category}-${Math.random()}`"
                class="inline-block font-['Barlow_Condensed'] font-medium text-sm sm:text-base text-gray-500 dark:text-gray-400 px-4">
                {{ category }}
              </span>
            </div>
          </div>
